package Lab5;
import java.util.Scanner;

class Employee7 {
    protected String name;
    protected double baseSalary;
    
    public Employee7(String name, double baseSalary) {
        this.name = name;
        this.baseSalary = baseSalary;
    }
    
    public double calculatePay() {
        return baseSalary;
    }
}

class SalesEmployee7 extends Employee7 {
    protected double commissionRate;
    
    public SalesEmployee7(String name, double baseSalary, double commissionRate) {
        super(name, baseSalary);
        this.commissionRate = commissionRate;
    }
    
    @Override
    public double calculatePay() {
        return baseSalary + (baseSalary * commissionRate);
    }
}

class Manager7 extends Employee7 {
    protected double fixedBonus;
    
    public Manager7(String name, double baseSalary, double fixedBonus) {
        super(name, baseSalary);
        this.fixedBonus = fixedBonus;
    }
    
    @Override
    public double calculatePay() {
        return baseSalary + fixedBonus;
    }
}

class PayrollProcessor {
    public static void process(Employee7 e) {
        System.out.println(e.name + " total pay: " + e.calculatePay());
    }
}

public class Lab5_7 {
	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in);
		
		String sName = sc.next();
		double sSalary = sc.nextDouble();
		double sRate = sc.nextDouble();

		String mName = sc.next();
		double mSalary = sc.nextDouble();
		double mBonus = sc.nextDouble();
		
		SalesEmployee7 sales = new SalesEmployee7(sName, sSalary, sRate);
		Manager7 manager = new Manager7(mName, mSalary, mBonus);
		
		PayrollProcessor.process(sales);
		PayrollProcessor.process(manager);
		
		sc.close();
	}
}
